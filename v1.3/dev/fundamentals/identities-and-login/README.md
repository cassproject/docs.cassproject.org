...
![Visual](/v1.3/identities-and-login-1.png)

This is also possible with PKI.

![Visual](/v1.3/identities-and-login-2.png)
...
![Visual](/v1.3/identities-and-login-3.png)
...
3. Verify the identities and contacts are available.
    ```js
    EcIdentityManager.ids //Should show one EcIdentity in an array
    EcIdentityManager.contacts; //Probably will show no contacts.
    ```
...
1. EcPpks can serialize to:
    * **PEM String** (PKCS#1 by default)
        ```js
        EcIdentityManager.ids[0].ppk.toPem();
        ```
    * **PKCS#1 PEM**
        ```js
        EcIdentityManager.ids[0].ppk.toPkcs1Pem();
        ```
    * **PKCS#8 PEM**
        ```js
        EcIdentityManager.ids[0].ppk.toPkcs8Pem();
        ```

2. EcPpks can also extract the public key portion as a EcPk object.
    ```js
    EcIdentityManager.ids[0].ppk.toPk();
    ```
...
1. EcPks can serialize to:
    ...
    * **PKCS#1 PEM**
        ```js
        EcIdentityManager.ids[0].ppk.toPk().toPkcs1Pem();
        ```
    * **PKCS#8 PEM**
        ```js
        EcIdentityManager.ids[0].ppk.toPk().toPkcs8Pem();
        ```
