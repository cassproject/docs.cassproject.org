...
Adapters run on the CASS Repository, a **LEVR** server. They may be written in one of two languages: a functional language called **RS2**, or a JS environment in LEVR called **LevrJs**. Because LevrJs can use the CASS Library, we recommend LevrJs for most cases.
...
If you would like to use the CASS Library in a non-LEVR environment, it is also possible to use a Node.js server with the CASS Library.
