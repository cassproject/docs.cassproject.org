...
Most CASS objects include helper functions include helper methods for retrieving objects of that type (e.g. ```EcCompetency.get()```).  However, when using the [EcRepository](http://cassproject.github.io/CASS/docs/classes/EcRepository.html) object to retrieve data (through ```.get()```, ```.getBlocking()```, and ```.multiget()```), all objects are returned as ```EcRemoteLinkedData``` types and must be cast into the proper CASS object.
...
p.addOwner(EcIdentityManager.ids[0].ppk.toPk());
```
...
p.addReader(EcIdentityManager.contacts[0].pk);
```
...
```js
var e = EcEncryptedValue.toEncryptedValue(p);
```
...
```js
EcRepository.save(e,console.log,console.log);
var myIds = EcIdentityManager.ids; //Saving our identities elsewhere.
EcIdentityManager.ids = []; //Removing them from the expected location.
EcRepository.cache = {}; //Clearing the cache.
EcRepository.getBlocking(e.id); //Not found
EcIdentityManager.ids = myIds;
EcRepository.getBlocking(e.id); //Found!
```

Now we can decrypt the object and see it is intact.

```js
var v = new EcEncryptedValue(); //First we have to cast it into an EcEncryptedValue
v.copyFrom(EcRepository.getBlocking(e.id));
var p = new Person(); //Then we decrypt it and cast it into the appropriate object.
p.copyFrom(v.decryptIntoObject());
```
...
```js
EcRepository.save(p,console.log,console.log);
```
...
```js
var r = EcRepository.getBlocking(p.id);
```
...
```js
var r;
EcRepository.get(p.id,function(o){r=o;},console.log);
```
