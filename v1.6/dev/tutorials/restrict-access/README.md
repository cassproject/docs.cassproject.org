...
On the topic of security, one may opt to restrict who, or what systems, can access a CaSS Repository. CaSS supports several options for this:
...
one may restrict all requests to the CaSS server by filtering by IP address. To do this, CaSS uses the `CASS_IP_ALLOW` environment variable. For example:

```bash
CASS_IP_ALLOW=127.0.0.1,28.28.28.0/24
```

Set this variable to a comma-separated list of IP addresses the user wish to permit to communicate with the server. If a request is made through an IP address not listed here, a 403 status code will be returned to the client (with the exception of certain routes).
...
A CaSS user may hold, or belong to, a number of identities. We may restrict access to only users who have a certain number of established identities. CaSS uses the `CASS_SSO_ACCOUNT_REQUIRED` environment variable to do this. For example, if we wish to specify only users with at least two identities:
...
CaSS supports access through [OpenID Connect](https://openid.net/connect/). one may use the preferred OpenID Connect provider, e.g. [Keycloak](https://www.keycloak.org/).

Modify the following environment variables to the current configuration. For example, for a Keycloak configuration we might use something like:
